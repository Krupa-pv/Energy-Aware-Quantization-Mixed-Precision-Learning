...
\author{
  Salma Bhar \quad
  Wiam Skakri \quad
  Krupa Venkatesan \\[0.5em]
  \texttt{\{sxb1283, wxs428, kxv178\}@case.edu}
}
...
\end{equation}
where $N_{\text{MAC}}$ is the number of multiply-accumulate operations, $E_{\text{MAC}}$ is the energy per MAC, $N_{\text{mem}}$ is the number of memory accesses, and $E_{\text{DRAM}}$ is the energy per memory access. Critically, memory access dominates energy consumption in modern systems, a 32-bit DRAM access consumes approximately 200$\times$ more energy than a 32-bit floating-point multiply~\cite{horowitz2014energy}.
...
We apply per-channel quantization for weights (separate scale per output channel) and per-tensor quantization for activations, following best practices~\cite{krishnamoorthi2018whitepaper}.
...
\begin{itemize}
    \item \textbf{MAC energy}: $E_{\text{MAC}}(b) \propto b^2$ as energy scales quadratically with bitwidth due to multiplier area scaling.
    \item \textbf{Memory energy}: $E_{\text{DRAM}}(b) \propto b$ as energy scales linearly with bitwidth due to data transfer volume.
\end{itemize}
...
\begin{table}[htb]
\centering
\caption{Quantization results for DeiT-Tiny on ImageNet.}
...
\textbf{Mixed-precision achieves the best accuracy-energy trade-off.} Our sensitivity-based mixed-precision configuration achieves 85.7\% energy reduction for ResNet-18 with only 0.64\% accuracy loss which is substantially better than 4-bit PTQ which sacrifices 23.4\% accuracy for an additional 2.5\% energy savings.
...
\subsection{Ranking-Aware Quantization for Vision Transformers}

To address the unique challenges of quantizing Vision Transformers, we implemented and evaluated ranking-aware quantization~\cite{liu2021ptqvit}, which preserves the relative ordering of attention scores during quantization. This is critical because the self-attention mechanism relies on ranking relationships rather than absolute values.

\begin{table}[htbp]
\centering
\caption{Comparison of standard PTQ vs. ranking-aware PTQ for DeiT-Tiny on ImageNet.}
\label{tab:ranking_aware}
\begin{tabular}{lccccc}
\toprule
\textbf{Bit-width} & \textbf{Standard PTQ} & \textbf{Ranking-Aware PTQ} & \textbf{Improvement} & \textbf{Top-5 Acc (RA)} \\
\midrule
8-bit & 85.10\% & 85.18\% & \textcolor{highlight}{+0.08\%} & 97.22\% \\
6-bit & 85.01\% & 85.11\% & \textcolor{highlight}{+0.10\%} & 97.18\% \\
4-bit & 83.04\% & 74.90\% & \textcolor{red}{-8.14\%} & 93.01\% \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:ranking_aware} presents our ranking-aware quantization results. Key observations:

\textbf{Consistent improvements at moderate precision.} Ranking-aware PTQ provides measurable improvements at 8-bit (+0.08\%) and 6-bit (+0.10\%), reducing accuracy loss by approximately 30-33\% relative to standard PTQ. This validates the hypothesis that preserving attention score rankings helps maintain model performance under quantization.

\textbf{Failure at extreme quantization.} At 4-bit precision, ranking-aware PTQ performs significantly worse than standard PTQ (-8.14\% difference), dropping from 83.04\% to 74.90\%. This occurs because 4-bit precision (only 16 discrete values) cannot simultaneously satisfy ranking constraints and represent the dynamic range needed for attention scores, leading to over-constrained optimization.

\subsection{Layer-Wise Sensitivity Analysis}

\begin{table}[htbp]
\centering
\caption{Top-5 most sensitive and most robust layers in DeiT-Tiny (49 quantizable layers total).}
\label{tab:layer_sensitivity}
\begin{tabular}{lcc}
\toprule
\multicolumn{3}{c}{\textbf{Most Sensitive Layers (MLP Feed-Forward)}} \\
\midrule
\textbf{Layer Name} & \textbf{Sensitivity Score} & \textbf{Type} \\
\midrule
...
\midrule
\multicolumn{3}{c}{\textbf{Most Robust Layers (Attention Projection)}} \\
\midrule
...
Table~\ref{tab:layer_sensitivity} reveals striking patterns in DeiT-Tiny's layer-wise quantization sensitivity. MLP feed-forward layers (especially fc2 layers in deeper blocks) are 4.7$\times$ more sensitive than attention projection layers. This suggests a clear strategy for mixed-precision allocation: \textbf{allocate 8-bit to MLP layers in blocks 5-9, and use 4-bit for attention projection layers.}
...
\subsection{Ranking-Aware Quantization: When It Works and When It Fails}

Our ranking-aware quantization experiments reveal a nuanced picture of transformer-specific quantization strategies. At 8-bit and 6-bit precision, preserving attention score rankings provides consistent improvements (+0.08-0.10\%), validating the core insight from~\cite{liu2021ptqvit} that relative orderings matter more than absolute values in the attention mechanism.
...
\subsection{Architectural Insights from Layer Sensitivity}

The 4.7$\times$ sensitivity gap between MLP layers and attention projections in DeiT-Tiny reveals fundamental architectural properties. MLP layers perform high-dimensional feature transformations (hidden\_dim = 4$\times$embed\_dim) with diverse weight distributions, making them vulnerable to quantization. In contrast, attention projection layers benefit from:

\begin{itemize}
    \item \textbf{Normalized inputs}: Attention outputs are softmax-normalized, providing bounded input ranges.
    \item \textbf{Linear operations}: Projections are pure linear transforms without non-linearities that amplify errors.
    \item \textbf{Residual connections}: Skip connections provide alternate gradient paths, reducing individual layer criticality.
\end{itemize}

This suggests a general principle: \textit{layers with high-dimensional transformations and diverse distributions require higher precision, while normalized linear projections can tolerate aggressive quantization.}
...
\begin{itemize}
    ...
    \item Ranking-aware PTQ improves accuracy by +0.08-0.10\% at 8/6-bit precision for Vision Transformers, but fails at 4-bit due to quantization grid coarseness.
    \item Layer sensitivity analysis reveals that MLP feed-forward layers are 4.7$\times$ more sensitive than attention projections, providing clear guidance for mixed-precision allocation.
    \item Mixed-precision assignment based on layer sensitivity provides favorable accuracy-energy trade-offs, achieving 85.7\% energy reduction for ResNet-18 and 80.9\% for DeiT-Tiny with minimal accuracy loss.
    \item Memory access dominates inference energy (93-99\%), making bitwidth reduction particularly effective for efficiency gains.
\end{itemize} \
...
% REFERENCES
%==============================================================================
% Bibliography (using manual thebibliography instead of BibTeX)
